...
export interface LegalPageFormValue {
  translations: { en: TranslationValue, id: TranslationValue }
}

function emptyTranslation(): TranslationValue {
  return { title: '', content: {}, seoDescription: '' }
}

const props = withDefaults(defineProps<{
  initial?: Partial<LegalPageFormValue>
  submitting?: boolean
  submitLabel?: string
  errorMessage?: string
}>(), {
...
const emit = defineEmits<{ submit: [value: LegalPageFormValue] }>()

const form = reactive<LegalPageFormValue>({
  translations: {
...
<template>
  ...
  >
    ...
    <div class="sticky bottom-0 z-20 -mx-4 flex items-center justify-between gap-4 border-t border-[var(--color-border)] bg-[var(--color-bg)]/95 px-4 py-4 backdrop-blur sm:-mx-6 sm:px-6">
      ...
      >
        Both language titles are required.
      </p>
      <span v-else />
      <BaseButton
        type="submit"
